...

{
    ...
    public class FurnitureManualJobOrderController : Controller
    {
        private ExceedDbContext db = new ExceedDbContext();

        public ActionResult Index()
        {
            ViewData["Proforma"] = db.FurnitureProformas.ToList();
            ViewData["JobType"] = db.FurnitureJobCategories.Select(s => new {
                JobTypeId = s.FurnitureJobCategoryId,
                type = s.HaveParent?db.FurnitureJobCategories.Where(q=>q.FurnitureJobCategoryId == s.FurnitureParentId).FirstOrDefault().JobName+"-"+ s.JobName : s.JobName
            }).ToList();
            ViewData["CustomerTypes"] = db.FurnitureProductionCustomerTypes.Select(s => new {
                Code = s.FurnitureProductionCustomerTypeId,
                Name = s.Name
            }).ToList();
            ...
            ViewData["Machines"] = db.FurnitureMachines.Select(s => new {
                Code = s.FurnitureMachineId,
                Name = s.MachineName
            }).ToList();
        ...
        {
            IQueryable<FurnitureJob> jobs = db.FurnitureJobs;
        ...
        {
            var proforma = db.FurnitureProformas.FirstOrDefault(x => x.FurnitureProformaId == proformaId);
        ...
        public ActionResult ChangeOrder(ChangeOfOrderVM changeOfOrder)
        {
            ...
            if (jobOrder != null)
            {
                ...
                {
                    var jobStatusLineItem = db.FurnitureJobStatusLineItems.Where(q => q.FurnitureJobStatusId == jobStatus.FurnitureJobStatusId).OrderByDescending(or=>or.FurnitureJobStatusLineItemId).FirstOrDefault();
                    if(jobStatusLineItem!=null)
                    {
                        jobStatusLineItem.RemainigQuantiy = changeOfOrder.ChangedQuantity;
                        db.FurnitureJobStatusLineItems.Attach(jobStatusLineItem);
