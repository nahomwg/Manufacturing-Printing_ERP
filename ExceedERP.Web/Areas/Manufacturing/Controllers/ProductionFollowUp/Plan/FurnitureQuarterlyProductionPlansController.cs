...
{
    public class FurnitureQuarterlyProductionPlansController : Controller
    {
        ...
        public ActionResult QuarterlyProductionPlans_Read([DataSourceRequest] DataSourceRequest request, int? id)
        {
            ...
            IQueryable<FurnitureQuarterlyProductionPlan> MarketPriceEntryPlans = db.FurnitureQuarterlyProductionPlans;
            if (id != null)
            {
                MarketPriceEntryPlans = MarketPriceEntryPlans.Where(q => q.FurnitureHalfYearlyProductionPlanId == id);
            }


            DataSourceResult result = MarketPriceEntryPlans.ToDataSourceResult(request, QuarterlyProductionPlan => new
            {
                QuarterlyProductionPlanId = QuarterlyProductionPlan.FurnitureQuarterlyProductionPlanId,
                HalfYearlyProductionPlanId = QuarterlyProductionPlan.FurnitureHalfYearlyProductionPlanId,
        ...
        public ActionResult QuarterlyProductionPlans_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")] IEnumerable<FurnitureQuarterlyProductionPlan> quarterlyPlans, int id)
        {
            if (quarterlyPlans != null && ModelState.IsValid)
            {
                var halfYearlyPlan = db.FurnitureHalfYearlyProductionPlans.Find(id);
                var quarterYearIds = quarterlyPlans.Select(s => s.FurnitureQuarterlyProductionPlanId);
                var qaurterYearProductionPlanNotUpdatedRecs = db.FurnitureQuarterlyProductionPlans.Where(q => q.FurnitureHalfYearlyProductionPlanId == id && !quarterYearIds.Contains(q.FurnitureQuarterlyProductionPlanId)).ToList();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var entity = db.FurnitureQuarterlyProductionPlans.Find(quarterlyPlan.FurnitureQuarterlyProductionPlanId);
                        if (entity != null)
                        {
                            ...
                            db.FurnitureQuarterlyProductionPlans.Attach(entity);
