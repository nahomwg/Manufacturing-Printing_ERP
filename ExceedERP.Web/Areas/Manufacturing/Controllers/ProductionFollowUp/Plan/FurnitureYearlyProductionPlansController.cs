...
{
    public class FurnitureYearlyProductionPlansController : Controller
    {
        ...
        public ActionResult YearlyProductionPlans_Create([DataSourceRequest] DataSourceRequest request, FurnitureYearlyProductionPlan yearlyPlan)
        {
            ...
            if (budgetYear != null)
            {
                var checkPlans = db.FurnitureYearlyProductionPlans.Where(q => q.GlFiscalYearId == budgetYear.GlFiscalYearId);
                if (checkPlans.Any())
                {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            db.SaveChanges();
                            var halfYearPlans = db.HalfYearlyProductionPlans.Where(q => q.YearlyProductionPlanId == entity.FurnitureYearlyProductionPlanId).ToList();
                            int i = 0;
                            foreach (var halfYearPlan in halfYearPlans)
                            {
                                ...
                                if (i == 0)
                                {
                                    ...
                                    {
                                        FurnitureHalfYearlyProductionPlanId = halfYearPlan.HalfYearlyProductionPlanId,
                            ...
                            var quarterPlans = db.FurnitureQuarterlyProductionPlans.Where(q => halfYearIds.Contains(q.FurnitureHalfYearlyProductionPlanId)).ToList();
                            ...
                            foreach (var quarterPlan in quarterPlans)
                            {
                                if (i == 0)
                                {
                                    ...
                                    {
                                        QuarterlyProductionPlanId = quarterPlan.FurnitureQuarterlyProductionPlanId,
                                        Name = "July",
                                        NameAm = "ሓምሌ",
                                        StartDate = startDate,
                                        StartDateEth = startDateEth,
                                ...
                                if (i == 1)
                                {
                                    ...
                                    {
                                        FurnitureMonthlyProductionPlanId = quarterPlan.FurnitureQuarterlyProductionPlanId,
                                        Name = "October",
                                        NameAm = "ጥቅምት",
                            ...
                            i = 0;
                            var quarterYearIds = quarterPlans.Select(s => s.FurnitureQuarterlyProductionPlanId);
